...
async function pollUntilDone(
  ...
  while (Date.now() - start < timeoutMs) {
    ...
    console.error(`[DEBUG] Poll job ${jobId}: state=${status.state}, messageCount=${status.messageCount}, recordCount=${status.recordCount}, pendingWarnings=${JSON.stringify(status.pendingWarnings)}`);
  ...
  server.tool(
    ...
    async ({ account, query, from, to, limit, byReceiptTime, renderChart, timeout }) => {
      try {
        ...
        const jobId = job.id;
        console.error(`[DEBUG] Created search job: ${jobId} for account: ${account}`);

        try {
          // 2. Poll until done
          const status = await pollUntilDone(client, jobId, account, pollTimeout);
          console.error(`[DEBUG] Job ${jobId} completed. Status: ${JSON.stringify(status)}`);
          ...
          if (status.recordCount > 0) {
            ...
            console.error(`[DEBUG] Job ${jobId} records response: ${JSON.stringify(records)}`);
            resultText = formatRecords(records);
            if (renderChart) {
            ...
            console.error(`[DEBUG] Job ${jobId} messages response: ${JSON.stringify(messages)}`);
            resultText = formatMessages(messages);
          }
